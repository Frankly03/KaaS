...
# --- New, Cleaner Prompt Template ---
PROMPT_TEMPLATE = """
You are a helpful and concise assistant. Answer the user's question in a natural, conversational way using only the provided context.
Do not mention the context or the sources in your answer. Just provide the answer directly.
If the context does not contain enough information to answer the question, simply say "I'm sorry, but I couldn't find enough information in the documents to answer that question."
...
def _generate_with_groq(prompt: str) -> str:
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {
                "role": "system",
                "content": "You are a helpful assistant that answers questions conversationally based only on the provided context."
            },
            {
                "role": "user",
                "content": prompt,
...
# --- Hugging Face Fallback (remains unchanged) ---
hf_pipeline = None
...
def _generate_with_hf(prompt: str) -> str:
    ...
    try:
        ...
        simple_prompt = f"Context: {context_part}\n\nQuestion: {question_part}\n\nAnswer:"
    except IndexError:
        simple_prompt = prompt
